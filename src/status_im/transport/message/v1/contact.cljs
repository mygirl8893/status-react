...
(defrecord NewContactKey [sym-key topic message]
  ...
  (receive [this chat-id signature {:keys [db] :as cofx}]
    (if (and (= (get-in cofx [:db :transport/chats chat-id :sym-key]) sym-key)
             (= (get-in cofx [:db :transport/chats chat-id :topic]) topic))
...
(defrecord ContactRequest [name profile-image address fcm-token]
  ...
  (receive [this chat-id signature {:keys [db] :as cofx}]
    (let [message-id (transport.utils/message-id this)
          topic      (get-in cofx [:db :transport/chats chat-id :topic])]
...
(defrecord ContactRequestConfirmed [name profile-image address fcm-token]
  ...
  (send [this chat-id cofx]
    (let [message-id                         (transport.utils/message-id this)
          {:keys [sym-key-id sym-key topic]} (get-in cofx [:db :transport/chats chat-id])]
      {:dispatch [:contact/send-new-sym-key
                  {:sym-key-id sym-key-id
                   :sym-key    sym-key
                   :chat-id    chat-id
                   :topic      topic
